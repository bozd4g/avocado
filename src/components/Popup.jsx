  ...
  initializePosts() {
        ...
        rows.push(
          <div>
            <MediaQuery query={g.maxWidth}>
              <div
                className="post"
                style={{ minWidth: "94%", marginBottom: "10%", padding: "3%" }}
              >
                <ContentLoader
                  speed={2}
                  height={75}
    ...
    if (lastCacheDate != null) {
      const dateDiff = Math.round(
        Math.abs(
          (new Date(lastCacheDate).getTime() - new Date().getTime()) /
            (24 * 3600 * 1000)
        )
      );
      if (dateDiff >= 1) localStorage.clear();
    }
    ...
    if (data == null) {
      axios
        .get(
          "https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fmedium.com%2Ffeed%2F%40bozd4g"
        )
        .then(res => {
          ...
          setTimeout(() => {
            this.bindLastPosts(
              res.data.items.filter(function(e) {
      ...
      rows.push(
        <div key={i}>
          <MediaQuery query={g.maxWidth}>
            <div
              className="post"
              style={{ width: "94%", marginBottom: "10%", padding: "3%" }}
            >
              <a href={d[i].guid} rel="noopener noreferrer" target="_blank">
                <h2 style={{ fontSize: "1.4em", width: "100%" }}>
                  {this.replaceAll(d[i].title, "&amp", "&")}
                </h2>
              </a>
              <p style={{ width: "100%" }}>
                {this.replaceAll(
                  d[i].description.substring(0, 200),
                  /(<([^>]+)>)/,
                  ""
                )}
                ..
                <a href={d[i].link} rel="noopener noreferrer" target="_blank">
                  read more
                </a>
              </p>
            </div>
          ...
          <MediaQuery query={g.minWidth}>
            <div className="post">
              ...
              <p>
                {this.replaceAll(
                  d[i].description.substring(0, 400),
                  /(<([^>]+)>)/,
                  ""
                )}
                ..
